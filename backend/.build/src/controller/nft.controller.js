...
const umi = (0, umi_bundle_defaults_1.createUmi)("http://api.devnet.solana.com")
...
const masterWallet = (0, umi_1.createSignerFromKeypair)(umi, keypair);
...
const createApiCollection = async (req, res) => {
    try {
        ...
        await (0, mpl_core_1.createCollection)(umi, {
...
const mintApiNFT = async (req, res) => {
    try {
        if (!req.file) {
        ...
        if (collectionId) {
            try {
                const collection = await (0, mpl_core_1.fetchCollection)(umi, (0, umi_1.publicKey)(collectionId));
        ...
        const createResult = await (0, mpl_core_1.create)(umi, mintParams).sendAndConfirm(umi);
        //save nft to database
        await db_1.db.insert(nft_1.nft).values({
...
const getAssetData = async (req, res) => {
    try {
        ...
        const asset = await (0, mpl_core_1.fetchAsset)(umi, (0, umi_1.publicKey)(assetId));
        res.json({
            success: true,
            asset: {
                id: asset.publicKey,
    ...
    try {
        ...
        for (const address of userAddresses) {
            try {
                ...
                const assets = await fetchAssetsByOwner(umi, (0, umi_1.publicKey)(address.address));
...
const transferNft = async (req, res) => {
    try {
        ...
        // Transfer the asset
        const transferResult = await (0, mpl_core_1.transfer)(umi, {
...
const getPlatformNFTs = async (req, res) => {
    try {
        const { fetchAssetsByOwner } = require("@metaplex-foundation/mpl-core");
